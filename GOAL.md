...
## 5. Sprint 1 之后的路线图（供后续接手人参考，不是现在要做的事）

- **Sprint 2**：给 agy 适配器加 `--background` / `/agy:status`，这时候才第一次需要一个 job 记录文件——独立实现，不 import codex-plugin-cc 的 `state.mjs`。
